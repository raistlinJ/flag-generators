...
VARIANTS: dict[str, dict[str, Any]] = {
    ...
    "http_support_ticket_portal": {
        "scheme": "http",
        "auth": "none",
        "title": "Helpdesk Ticket Portal",
        "tagline": "Support desk where each ticket page carries its reference and attachment.",
        "theme": "tickets",
        "endpoint": "/tickets/",
        "flag_path": "tickets/ticket-note.txt",
        # The ticket reference is the point of this generator: it is the only
        # producer of Ticket(id) in the catalog, and dep_ticket_attachment_portal
        # consumes it as `ticket`/`X-Ticket-ID`. Without a producer that consumer
        # can never be satisfied by any chain, whatever the ordering.
        "extra_facts": {"Ticket(id)": "HD-48213", "Endpoint(path)": "/tickets/HD-48213"},
        "files": {
            "tickets/ticket-note.txt": (
                "Helpdesk ticket HD-48213 {digest}\n"
                "status=escalated\n"
                "attachment=ticket-HD-48213-evidence.txt\n"
                "flag={flag}\n"
            ),
        },
    },
